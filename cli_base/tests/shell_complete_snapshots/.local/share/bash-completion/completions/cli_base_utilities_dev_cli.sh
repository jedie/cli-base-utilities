...
# Bash completion for ./dev-cli.py
...
__tyro_tyro___dev_cli_py_ltrim_colon_completions() {
  local cur=$1
...
PYTHON_EOF
  ...
  __tyro_tyro___dev_cli_py_ltrim_colon_completions "$cur"

  return 0
}

# Register completion for the full program name.
complete -F _tyro___dev_cli_py ./dev-cli.py

# Also register for basename to handle ./script invocations.
_prog_basename=$(basename "./dev-cli.py")
if [ "$_prog_basename" != "./dev-cli.py" ]; then
  complete -F _tyro___dev_cli_py "$_prog_basename"
fi
